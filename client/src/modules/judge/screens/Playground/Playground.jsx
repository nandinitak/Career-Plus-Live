...
const Playground = () => {
  ...
  const postSubmission = async (language_id, source_code, stdin) => {
    const options = {
      ...
      headers: {
        ...
        "Content-Type": "application/json",
        "X-RapidAPI-Key": `${import.meta.env.VITE_JUDGE_RAPID_API_KEY}`,
  ...
  const runCode = async () => {
    ...
    const status_name = res?.statusText;
    const decoded_output = decode(res?.stdout ? res?.stdout : "");
    const decoded_compile_output = res?.data?.stdout ? res?.data?.stdout : "";
    const decoded_error = decode(res?.stderr ? res?.stderr : "");
    console.log(decoded_compile_output);
    let final_output = "";
    if (res?.status_id !== 3) {
